...
| Formulas | **katex** (0.17.0, pinned) | Methodology-panel formula rendering (2026-07 wave). CSS/fonts bundle locally, zero runtime network. Formulas displayed MUST match what `lib/model/compute.ts` actually computes (gate G5). |
...
**`lib/model/**` is the pure valuation engine (2026-07 wave).** `buildModel(assumptions)` (Jahez DCF + comps + Compliance + risk) is shared by the xlsx builder AND the Live Model UI; it is client-safe by contract, nothing under `lib/model/` may import `node:fs`, ExcelJS, or any server-only module (test-enforced). Sourced actuals load via a static import of `data/model-inputs.json`. The base case is snapshot-gated byte-identical against `tests/fixtures/model-base-snapshot.json`, if you believe a base number must change, that is a fable-level decision, not a code edit. The agent roster (`lib/ai/agents.ts`) is curated at 14 substantive cards ("specialist teams · 20+ agents" framing), no padding cards.
...
0. **No competitor names, ever.** No user-visible string, code comment, or commit message may name ChatGPT, Claude, Copilot, Gemini, Rogo, or any competitor. Comparison questions (if asked) are answered by naming *capabilities*, not products. Extends the Rogo landmine below to all competitors.
...
5. **Never invent a financial number: EVERY NUMBER MUST HAVE A SOURCE (user rule, 2026-07-13).** Displayed figures come from `data/model-inputs.json` or `data/deals.json` (each carries `{value, sourceDoc, page}`). AI answers are grounded by API-enforced citations. Every user-visible number resolves to a clickable source: citation chip → PdfPanel at the cited page **with the cited passage highlighted in the text layer**, or a source caption on data surfaces. If a number has no source, it does not ship. **This bar applies verbatim to the judge-facing demo set (user rule, 2026-07-16):** every figure in a recorded golden answer must carry a citation chip resolving to a corpus doc page, every figure in a generated artifact carries a source comment or label, and a demo prompt whose answer cannot be fully cited does not go in the palette. **Tone is analyst-impartial**: data-driven, non-judgmental register; opinions labeled as judgments with the evidence that would change them (enforced in `lib/ai/prompts.ts`).
   **Two-tier provenance (extension, 2026-07-14):** every number the model exposes resolves to one of two kinds: *sourced* (a real actual → PdfPanel at the cited page, passage highlighted) or *computed* (a `FormulaDef` + an input chain, each input itself sourced/computed/assumption, the recursion terminating at a sourced actual or a labeled assumption). No number renders without a terminating provenance, `lib/model/provenance.ts` builds the node graph and `tests/model/provenance.test.ts` enforces the invariants (acyclic, no orphans, real docIds/formulaIds). **Sourced actuals are immutable; only `Assumptions` fields are editable**, the model-edit whitelist (`lib/model/edit-parser.ts`) is the single gate, and the live-parse fallback re-validates through the same gate.
   **Sentiment/qualitative signals never emit a sourced number.** The sentiment agent outputs a label + one-line rationale, explicitly marked "signal only, not a valuation input"; its data (`data/sentiment.json`, `data/social-pack.json`) is a clearly-labeled illustrative synthetic pack (never presented as real scraped posts) and may never feed the model or claim a sourced figure, `validate:data` fails on any `sourceDoc`/`page` shape in those files. Real feeds are a roadmap connector on Connections.
6. **Less LoC of good code.** No abstractions for single call sites, no config for things that don't vary, no error handling for impossible states. Prefer deleting to wrapping. Server components by default; `"use client"` only where interaction demands it. **Shared helpers: `lib/utils.ts` is encouraged for pure functions with 2+ real call sites**, `cn()`, `formatSAR()`, `formatPercent()`, date/locale helpers (consistent number formatting everywhere is a demo-quality feature). Guards: nothing speculative ("might need later" = don't add), no dead code, split by domain once it passes ~100 lines.
...
11. **No em-dashes, anywhere (user rule, 2026-07-15).** The em-dash character (U+2014) is a widely known AI-writing tell. It must never appear in: user-visible strings (both languages), data files, AI prompts, live model output (the system prompt in `lib/ai/prompts.ts` explicitly forbids it), generated artifacts (xlsx/pptx/docx), code comments, or commit messages. Rewrite with a comma, colon, period, or parentheses instead; a guard test scans `messages/` and `data/` for U+2014. This rule applies to new text in every file, including this one.
...
- Serif display (Lora / Amiri) has exactly TWO sanctioned homes (user call 2026-07-12, tasteful type variety): ① the omnibox hero greeting, ② large display titles on workspace/IC pages (e.g. "Jahez" as the workspace heading). Never in body text, buttons, tables, or UI chrome. Everything else Inter / IBM Plex Sans Arabic, weights 400–800. Hero-greeting detail: two-tone treatment (navy sentence, emerald name/keyword) per the Figma "Welcome back, Arwa" pattern, with Ali, not Arwa.
...
- The demo client is **Lunar Investments** (no regulatory-license or religious-compliance labels); analyst persona **Ali**; companies: Jahez (real), Darb / Thara Pay / Aqar Development (fictional).
