...
5. Arabic register must match `context/pitch-deck-notes.md` vocabulary — e.g. الذكاء التوكيلي, لجنة الاستثمار, الفحص الشرعي, نقطة المراجعة البشرية. Use the AR strings given in each brief verbatim.
...
5. Shariah screen required — Toggle, default **ON**, caption 12/500 below: "Per AAOIFI methodology".
...
| `onboarding.mandate.shariah` | Shariah screen required | الفحص الشرعي إلزامي |
| `onboarding.mandate.shariahNote` | Per AAOIFI methodology | وفق منهجية أيوفي (AAOIFI) |
...
| Verification & Compliance | التحقق والامتثال | `shield-check` | Shariah screening (AAOIFI) · Fact-check vs source · Sanctions & conflicts · Confidence flags | `@compliance` |
