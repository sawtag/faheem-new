...
- [ ] Shell: Rogo-layout sidebar (CATALOG §2A) — logo, New Chat, nav (Home, Deals, Library, Agents, Scheduled Tasks visual, Connections), pinned workspaces from deals.json, Ali footer; language toggle (cookie + `dir` swap, animated); collapse.
...
- **Acceptance:** e2e — unauthenticated `/` → `/login`; submit "ali"/"demo" → home; empty submit → inline error, no redirect.
