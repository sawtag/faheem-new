...
- **Pre-installed tooling (user-level, 2026-07-12) — available, not mandated; you and your agents choose when they help:** skills `tailwind-design-system`, `nextjs-app-router-patterns`, `vercel-react-best-practices`, plus the environment's `frontend-design` and `dataviz`. The one hard rule (correctness, not preference): **never guess post-training APIs** (Next 16.2 / Tailwind 4.3 / radix-ui 1.6 / motion 12). Mechanism: web-search the official docs first, and **delegate doc lookups to a cheap subagent** so the payload never lands in the orchestrator's context — only the answer comes back. MCP `context7` exists as a fallback but **use it sparingly** (user had bad experiences; it floods context and blows up token usage).
