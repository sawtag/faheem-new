...
| 4.5–6 | **Private pipeline — Screening beat**: the deal-pipeline board (filter: **Inbound (Private) / Market Screen (Public)**) — **Qafilah** (logistics SaaS, *Screening*), **Thara Pay** (*IC Review*), **Aqar Development** (*Declined — outside mandate: real estate*), Jahez (*Analysis*, public badge). Open Qafilah's workspace: inbound request, docs the **Screening Agent** collected from the company ("details anonymized — client confidentiality"), the **mandate-fit scorecard** — sector ✓, ticket ✓, stage ✓, Shariah pre-screen ✓, concentration ⚠ — every criterion citing the **Lunar IC Charter** page. Click **"Advance to pitch meeting"** — human gate #1. | Live app |
...
1. **App shell / sidebar** — Faheem logo, New Chat, Home, Search, **Deals** (pipeline), Documents (Library), Agents, Scheduled Tasks (visual), pinned workspaces (Jahez, Qafilah, Thara Pay), user footer (Arwa avatar, "Lunar Investments"). Present on every screen.
...
│  ├─ deals.json                      # pipeline: company, sector, stage, origin (inbound|market-screen), ask, status line, screening scorecard rows (Qafilah), IC metrics (Jahez, Thara Pay), decline reason (Aqar)
...
12. **Qafilah data-room mini pack** (~6–8 pages total): one-page company profile + ask (Series B, SAR 40M, logistics SaaS), financial summary table, founder bios — "collected by the Screening Agent"
...
| | d) lunar-docs + fictional deals: IC Charter (15% hurdle, 10% concentration, 3–5y hold, sector mandate), Risk Appetite, Portfolio snapshot, **Qafilah data-room mini pack, Thara Pay analysis summary** → branded PDFs + `data/deals.json` (pipeline stages, screening scorecard rows, IC metrics) | sonnet / medium | |
...
| Criterion | Check | Example (Qafilah) |
|---|---|---|
...
| Series B | A company's second major venture funding round (post product-market fit, scaling) | Qafilah's ask |
| Data room | The secure folder of due-diligence documents a company shares with investors | Qafilah workspace, Connections |
