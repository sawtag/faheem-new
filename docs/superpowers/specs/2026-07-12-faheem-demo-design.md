...
| Demo narrative (2026-07-12 rev.) | **Deal-pipeline story: Screening → Deep Analysis → IC**, hybrid public/private | Mirrors the real firm workflow (practitioner-validated). Jahez stays the deep-dive centerpiece (real verified data, public-screening origin); fictional **Masar** (logistics SaaS, Series B inbound) carries the private screening beat; fictional **Thara Pay** (fintech, analysis complete) joins Jahez in the IC ranking; **Aqar Development** shown Declined (real estate — outside mandate). Fictional names are placeholders — rename freely. |
...
**Persona:** Arwa, analyst at Lunar Investments. **Frame:** one morning in the firm's real workflow — Screening → Deep Analysis → IC — with Faheem accelerating every stage and a human deciding at every gate. (Timings tight; tune in rehearsal.)
...
| 3–4 | **The workflow** (one slide): how an investment firm actually works — inbound pitch (or public screen) → mandate screening → weeks of manual deep analysis → Investment Committee. "Faheem doesn't replace this process or its people. It accelerates each stage from weeks to hours — and a human decides at every gate." | Slides |
| 4–5.5 | **Pipeline + Screening beat**: the Lunar deal-pipeline board — **Masar** (logistics SaaS, *Screening*), **Jahez** (*Analysis*), **Thara Pay** (*IC Review*), **Aqar Development** (*Declined — outside mandate: real estate*). Open Masar's workspace: inbound request, docs the **Screening Agent** collected from the company, and its **mandate-fit scorecard** — sector ✓, ticket size vs mandate ✓, Shariah pre-screen ✓, concentration ⚠ — every criterion citing the **Lunar IC Charter** page. Click **"Advance to pitch meeting"** — the human gate. One-liner while here: firm setup (connectors, mandate questionnaire, brand templates) happened at onboarding — flash the Connections page; note Jahez entered the pipeline via the **public** variant: a SAHMK/Argaam market screen, no inbound needed. | Live app |
| 5.5–8.5 | **Deep dive Q&A 1 (Jahez — the money shot)**: in the Jahez workspace: "Break down Jahez's FY2025 unit economics from **#FY2025-Earnings-Release** — GMV growth vs take rate, AOV, contribution margin, EBITDA margin — and why did net income compress ~61% despite double-digit GMV growth?" (the `#` typeahead scoping the doc = 2-second flourish) → agent activity timeline (Research → Document Intelligence → **Valuation & Modeling** → **Risk & Portfolio Monitoring** → Compliance fact-check) → streamed analyst answer with inline numbered citation chips → click a citation → **the actual Jahez earnings PDF opens at the cited page**. Sources accordion shows grouped fact→citation mapping. | Live app |
| 8.5–10 | **Q&A moment 2**: "**@Risk & Portfolio Monitoring** — run a quantified risk assessment of the quick-commerce price war; does the position fit our mandate?" (the `@` typeahead = the multi-agent system is addressable) → **risk scorecard** (probability × impact, bps of take-rate compression per scenario, scenario-weighted expected return) citing BOTH the industry pack AND Lunar's private docs ("breaches your IC Charter's 15% IRR hurdle in the bear case; exceeds the 10% single-name concentration limit — Lunar IC Charter, p.4"). Prompt-improve moment: type "jahez good?" → **Improve** → full structured equity-research prompt (Haiku). | Live app |
| 10–11 | **Arabic moment**: toggle the whole UI to Arabic live (full RTL) and ask: "هل يجتاز جاهز الفحص الشرعي وفق منهجية AAOIFI؟" → Shariah-screen card (debt/market-cap ratio, non-permissible income %, pass/fail) answered in Arabic with citations. Bilingual product + the most Saudi-native analysis in one beat. | Live app |
| 11–12.5 | **Deliverables**: "Prepare the IC memo, DCF model, and committee deck." → generation progress → **Word IC memo + Excel valuation workbook + PPTX board deck, all in Lunar Investments branding** → open the Excel live: Assumptions, Revenue Drivers (GMV → take rate → net revenue), 3-Statement, **DCF (FCFF, WACC build, Gordon terminal value)**, **Sensitivity (WACC × terminal growth)**, **Comps (vs Talabat, Deliveroo, DoorDash)**, Scenarios (bull/base/bear with implied IRR) — every populated cell carries "Source: Jahez FY2025 Earnings Release, p.3" linking back into Faheem's viewer. Artifacts land in the Jahez workspace Library. | Live app + Office |
| 12.5–13.5 | **Faheem IC room**: the committee view — comparison table of analysis-complete deals (**Jahez vs Thara Pay**: implied IRR vs 15% hurdle, risk score, mandate fit, Shariah). Ask **Faheem IC**: "Rank these against our hurdle — where's the strongest risk-adjusted case?" → advisor answer with citations into both companies' analyses + the on-screen disclaimer: **"Advisory only — the investment decision rests with the committee."** The third human gate. | Live app |
| 13.5–15 | **Close**: architecture recap (judges already SAW the agents live — one slide: orchestrator, 7 teams, fact-checker, 3 human gates); Connections/MVP roadmap (data rooms — Intralinks/Datasite, Bloomberg/PitchBook, od.data.gov.sa, SAMA open banking); business model (SaaS tiers/seat + usage); Alinma synergy (Faheem for Alinma Capital's research desk). "We're not replacing analysts — we're accelerating weeks into hours. We're not building a chatbot — we're building the knowledge infrastructure of the Saudi financial sector." Team + ask. | Slides |
...
1. **App shell / sidebar** — Faheem logo, New Chat, Home, Search, **Deals** (pipeline), Documents (Library), Agents, Scheduled Tasks (visual), pinned workspaces (Jahez, Masar, Thara Pay), user footer (Arwa avatar, "Lunar Investments"). Present on every screen.
...
9. **Onboarding wizard** (demoted from the demo — referenced in one line during the pipeline beat; build only if time allows, else a static "Settings → Firm profile" page shows the same data): ① Company profile & subscription tier ② Connect data sources (fake OAuth modals) ③ Brand & templates upload ④ **Investment mandate & risk appetite questionnaire in IC language** — target IRR / hurdle rate 15%, max single-name concentration 10%, holding period 3–5y, liquidity, mandatory Shariah screen, sector appetite (Saudi consumer internet, fintech & logistics tech), max drawdown. This data is REAL regardless of the wizard — it IS the Lunar IC Charter corpus doc the Screening Agent and Q&A 2 cite.
...
│  ├─ deals.json                      # pipeline: company, sector, stage, ask, status line, screening scorecard rows (Masar), IC metrics (Jahez, Thara Pay), decline reason (Aqar)
...
9. **Lunar IC Charter & Investment Mandate** (allocation limits, sector appetite — Saudi consumer internet/fintech/logistics tech, 15% IRR hurdle, 10% single-name concentration, mandatory Shariah screen) — now triple-duty: cited by the Screening Agent scorecard, Q&A 2 mandate-fit, and Faheem IC
...
12. **Masar data-room mini pack** (~6–8 pages total): one-page company profile + ask (Series B, SAR 40M, logistics SaaS), financial summary table, founder bios — "collected by the Screening Agent"
...
| | d) lunar-docs + fictional deals: IC Charter (15% hurdle, 10% concentration, 3–5y hold, sector mandate), Risk Appetite, Portfolio snapshot, **Masar data-room mini pack, Thara Pay analysis summary** → branded PDFs + `data/deals.json` (pipeline stages, screening scorecard rows, IC metrics) | sonnet / medium | |
...
| **P3 Screen fan-out** | deal pipeline board + company workspaces · Faheem IC room · connections · agents page (stage-grouped) · library/deliverables UI · onboarding wizard (last, cuttable) — one subagent each, disjoint routes, shared components + chat engine frozen first; each agent gets the §7 tokens + relevant `context/rogo-screens/CATALOG.md` excerpts in its brief | 6× sonnet / medium (parallel) | pipeline/workspace/IC render from `deals.json`; main reviews the two demo-critical ones (pipeline, IC) before integration; degrade order per §9 |
...
| Time overrun | Screens 1–6 are the demo; 7–9 degrade to static mockups if needed. Further degradation order: onboarding wizard → agents-page interactivity (static diagram) → library polish → workspace tabs collapse to Overview+Chats. The Jahez deep dive + citations are never cut. |
...
| Criterion | Check | Example (Masar) |
|---|---|---|
| Sector mandate | in/out of "Saudi consumer internet, fintech & logistics tech" | ✓ logistics SaaS |
...
| Series B | A company's second major venture funding round (post product-market fit, scaling) | Masar's ask |
| Data room | The secure folder of due-diligence documents a company shares with investors | Masar workspace, Connections |
