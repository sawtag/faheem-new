...
- 🗣️ **Tell the model what to change, in plain language.** "Raise FY26 order growth to 20%" recomputes the DCF live while the specialist team works in the open — Valuation re-derives, Critical Review re-verifies the provenance chain, Compliance re-checks the Shariah ratios, Writing updates the recommendation. Sourced actuals stay immutable: try to edit one and Faheem explains why, gracefully, instead of pretending to comply.
...
- 📄 **Bring your own IC memo template.** Upload the firm's own Word template (`{{tag}}` placeholders) on the Library page and the IC memo generator fills it with live model values instead of the built-in memo — a sample template and a live tag catalog are one click away. No template uploaded, no change: generation stays byte-identical to the built-in memo.
...
| Company Word template for the IC memo — upload `{{tag}}` template, live tag catalog, generator fills it instead of the built-in memo                                   | extra      | ✅ Implemented |
