...
- **A complete demo**, `master` @ `demo-rc2`: login → onboarding → pipeline + Darb screening scorecard → Jahez chat with citations → Arabic/Compliance beat → deliverables (xlsx/docx/pptx + in-app preview) → IC room → dashboard (with an "Analysis Runs" panel) → audit trail → skills library → live PDF upload. Bilingual EN/AR, full RTL.
